...
<template lang="jade">
  .group-page
    ...
    .stock-list.scroll-content
      .form
        .form-filters.my-el
          span
            ...
            el-date-picker(v-model='stEt', format='yyyy-MM-dd', type='daterange', placeholder='选择日期范围', v-bind:clearable='clearableOnTime')
            el-button(@click="stEt=['', '']", size='small') 不限
            el-button(@click='stEt=[new Date()._bf(-7), new Date()]', size='small') 最近七天
            el-button(@click='stEt=[new Date()._setH(0)._setM(0)._setS(0), new Date()._setH(23)._setM(23)._setS(59)]', size='small') 今天
          span
            | 状态 
        ...
        el-table.header-bold.nopadding(:data='data', stripe='stripe', v-bind:max-height=' MH ', v-bind:row-class-name='tableRowClassName', ref='table')
          ...
          el-table-column(prop='registertime', label='注册时间')
            template(scope='scope')
              span {{new Date(scope.row.registertime)._toString()}}
          el-table-column(prop='subCount', label='团队人数')
          el-table-column(prop='beginTm', label='签约日期')
          el-table-column(prop='sendCycle', label='其它游戏分红周期')
            template(scope='scope')
              span {{TIME[scope.row.sendCycle]}}
          el-table-column(prop='STYPE', label='发放方式')
            template(scope='scope')
              span {{STYPE[scope.row.sendType]}}
          ...
          el-table-column(label='操作')
            template(scope='scope')
              .ds-button.text-button.blue(v-if=" scope.row.stat !== '未签订' ", style='padding: 0 .05rem;', @click.stop=' (showDetail = scope.row.id ,Ismyself = me.account==scope.row.userName?!0:!1) ') 查看详情
              .ds-button.text-button.blue(v-if="  ruleCfg.length > 0 && scope.row.stat === '未签订' ", style='padding: 0 .05rem;', @click='++stepIndex && (user = scope.row)') 新建契约
              .ds-button.text-button.blue(v-if="me.account!=scope.row.userName && ruleCfg.length > 0 && (scope.row.stat === '已签订' || scope.row.stat === '已拒绝' || scope.row.stat === '待确认')", style='padding: 0 .05rem;', @click='++stepIndex && (user = scope.row)') 重新发起
      ...
      // 弹窗
      .modal(v-if='stepIndex === 1')
        .mask
        .box-wrapper
          .box(ref='box', style='max-width: 9rem; max-height: 10rem; height: 6.2rem;')
            .tool-bar
              span.title 其它游戏分红详情
              el-button-group
                el-button.close(icon='close', @click='stepIndex = 0')
            div
              ...
              div(style='margin: 0 10% 0 25%; margin-top: .3rem; min-width: 6rem;', v-bind:class='[ user.state ]')
                ...
                p.item.block
                  ...
                  el-select(v-model=' SV ', style='width: .9rem;', placeholder='无')
                    el-option(v-for='S in sendCycle', v-bind:label=' TIME[S] ', v-bind:value='S')
                ...
                p.item.block(v-for=' (CR, i) in CRULES ')
                  ...
                  el-input-number.text-danger.text-right(style='width: .6rem;', v-model='CR.actUser', v-bind:debounce='2000')
                  ...
                  span(v-if='CR.bounsRate')
                    | 最低
                    span.text-blue
                      | {{ ruleCfg.find(x => x.ruletype === CR.ruletype && CR.bounsRate === x.bounsRate).sales }}
                    | 万，
                    span.text-blue
                      | {{ ruleCfg.find(x => x.ruletype === CR.ruletype && CR.bounsRate === x.bounsRate).actUser }}
                    | 人
...
export default {
  ...
  data() {
    return {
      ...
      r: { id: 1, title: "亏损" },
      RULES: [
        { title: "规则一", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则二", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则三", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则四", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则五", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则六", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则七", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则八", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则九", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十一", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十二", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十三", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十四", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十五", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十六", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十七", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十八", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则十九", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        { title: "规则二十", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 },
        {
          title: "规则二十一",
          ruletype: 1,
          sales: 0,
        ...
        { title: "规则三十", ruletype: 1, sales: 0, bounsRate: 0, actUser: 0 }
      ],
      ruleLength: 3,
      name: "",
      showDetail: false,
