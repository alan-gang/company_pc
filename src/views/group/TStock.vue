    ...
    .stock-list.scroll-content
      .form
        ...
        el-table.header-bold.nopadding(:data='bonusList', ref='table', stripe='stripe', show-summary='show-summary', v-bind:summary-method='getSummaries', v-bind:max-height=' MH ', v-bind:row-class-name='tableRowClassName')
          ...
          el-table-column(prop='startDate', label='测算开始时间')
          ...
          el-table-column(prop='bonus', label='个人预期分红金额')
            template(scope='scope')
              span {{ scope.row.bonus && scope.row.bonus._nwc() }}
        ...
        p(style='margin: 10px;font-size: 12px;color: #999;') 温馨提示：预期分红是基于阶段性数据测算得出,仅为预测下一次分红提供参考,不作为发放分红依据。
      // 弹窗 规则列表
            ...
            .ruleInfoLists
              ul
                li(v-for='(v, i) in ruleInfoListRow.bounsruleListBy', :class="{'on':v.id==ruleInfoListRow.ruleid}", :key='v.id')
                  | {{RULES[i]}}：累计{{TYPE[v.ruletype]}}{{v.sales}}万，有效人数>{{v.actuser}}，分红比例{{v.bounsrate}}%
