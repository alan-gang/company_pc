      ...
      <div v-if=" I === 0 ">
        <div class="form form-filters">
          <label class="item">
            ...
            <el-button @click="ClickMonth(1)" size="small">{{getLastMonth(0).month}}月</el-button>
            <el-button @click="ClickMonth(2)" size="small">{{getLastMonth(1).month}}月</el-button>
            <el-button @click="ClickMonth(3)" size="small">{{getLastMonth(2).month}}月</el-button>
          </label>
          <label class="item">
            排序
...
export default {
  ...
  methods: {
    ...
    ClickMonth(multiple) {
      ...
      r.push(
        new Date(
          `${this.getLastMonth(multiple).year}-${
            this.getLastMonth(multiple).month
          }-01`
        )._toDayString()
      );
      r.push(
        new Date(
          `${this.getLastMonth(multiple).year}-${
            this.getLastMonth(multiple).month
          }-${this.getCountDays(this.getLastMonth(multiple).month)}`
        )._toDayString()
      );
      // console.log(222, r);
      this.stEt = r;
