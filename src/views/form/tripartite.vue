      ...
      div(v-if=' I === 0 ')
        ...
        .table-list(style='padding: .15rem .2rem ;')
          ...
          div(v-show="profitmark === 'list'")
            el-table.header-bold.nopadding(:data='data', style='margin: 0;', ref='table', stripe='stripe', v-bind:row-class-name='tableRowClassName', v-bind:max-height=' MH ', @sort-change='sortChange')
              ...
              el-table-column(prop='userpoint', label='操作', align='center')
                template(scope='scope')
                  // 最后一条合计 不显示操作按钮
                  .ds-button.text-button.blue(v-show='Daily && scope.$index+1 != data.length', @click='ClickProfitInfo(scope.row)', style='padding: 0 .05rem;') 明细
          div(v-show="profitmark === 'info'")
            // 三方团队明细  月
            // 三方团队明细  日
            // v-bind:summary-method="getSummaries"
            el-table.header-bold.nopadding(:data='data', style='margin: 0;', ref='table2', stripe='stripe', v-bind:row-class-name='tableRowClassName', v-bind:max-height=' MH ', @sort-change='sortChange')
              ...
              el-table-column(align='right', prop='settle', :label="Daily ? '结算' :  '盈亏'", sortable='custom', class-name='pr2')
                template(scope='scope')
                  span(v-show='!Daily', :class=" {'text-green': scope.row.settle && scope.row.settle._o0(), 'text-danger': scope.row.settle && scope.row.settle._l0() } ") {{ numberWithCommas(scope.row.settle) }}
                  span(v-show='Daily', :class=" {'text-green': scope.row.totalProfit && scope.row.totalProfit._o0(), 'text-danger': scope.row.totalProfit && scope.row.totalProfit._l0() } ") {{ numberWithCommas(scope.row.totalProfit) }}
              el-table-column(prop='userpoint', label='操作', align='center')
                template(scope='scope')
                  // 最后一条合计 不显示操作按钮
            ...
            el-table.header-bold.nopadding(:data='cdata', stripe='stripe', ref='itable', max-height='500', v-bind:row-class-name='tableRowClassName', style='margin: .2rem 0 0 0;')
              ...
              el-table-column(align='right', prop='profit', label='游戏盈亏')
                template(scope='scope')
                  span(:class=" {'text-green': scope.row.profit && scope.row.profit._o0(), 'text-danger': scope.row.profit && scope.row.profit._l0() } ") {{ numberWithCommas(scope.row.profit) }}
              // me.showBackWater ‰
              el-table-column(align='right', label='返水级别', v-if='profitDetailROW && profitDetailROW.hasSub==0 && me.displayPermission.showback')
                template(scope='scope')
              ...
              el-table-column(align='right', prop='getpoint', label='返水金额', v-if='me.displayPermission.showback')
                template(scope='scope')
                  span {{ numberWithCommas(scope.row.getpoint) }}
              ...
              el-table-column(align='right', prop='settle', label='总盈亏', class-name='pr2')
                template(scope='scope')
                  span(:class=" {'text-green': scope.row.settle && scope.row.settle._o0(), 'text-danger': scope.row.settle && scope.row.settle._l0() } ") {{ numberWithCommas(scope.row.settle) }}
