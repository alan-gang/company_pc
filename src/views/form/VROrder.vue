    ...
    .user-list.scroll-content
      ...
      .form
        ...
        el-table.header-bold.nopadding(:data="Cdata" v-bind:row-class-name="tableRowClassName" v-on:row-click="setSelected" style="margin-top: .1rem")
          ...
          el-table-column(prop="serialNumber" label="注单编号" width="120" show-overflow-tooltip=true)
          ...
          el-table-column(label="状态" width="60")
            template(scope="scope")
              span(:class="{ 'text-danger': scope.row.state === 3,  'text-grey': scope.row.state === 0, 'text-green': scope.row.state === 2, 'text-black': scope.row.state === 1}") {{ STATUS[scope.row.state] }}
    ...
    methods: {
      ...
      Orderlist (page, fn) {
        ...
        this.$http.post(api.VROrderlist, this.preOptions).then(({data}) => {
          ...
          if (data.betRecords) {
            data.success = 1
            data.recordList = data.betRecords
